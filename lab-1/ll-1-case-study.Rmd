...
-   code is character variable with 2 categories
...
3793 observations vs 3034 in the training set
...
preds <- final_fit %>%
    collect_predictions() %>%
     mutate(correct = .pred_class == code)

preds %>% roc_curve(code, .pred_SB) %>% autoplot()
```
...
-   The percent of rows in the test set that did not match and did match between their predicted and and actual code.
...
-   This model worked pretty well, 87% correct from just these few indicators! The accuracy seems pretty good and the inaccuracy is balanced between the two categories.

-   The ROC AUC is still alive every time I rhyme. This metric looks quite good too, the ROC looks like what you'd want but my intuition about this is not as strong.
